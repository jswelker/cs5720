...
model.add(
    Conv2D(
        32,
        (3, 3),
        input_shape=(32, 32, 3),  # maybe needs batch_size first?
        padding="same",
        activation="relu",
...
model.add(Flatten())
# model.add(Dropout(0.2))
## OLD model.add(Dense(512, activation="relu", kernel_constraint=MaxNorm(3)))
## NEW model.add(Dense(1024, activation="relu", kernel_constraint=MaxNorm(3)))
## OLD model.add(Dropout(0.5))
## NEW model.add(Dropout(0.2))
